...
void write_data(void)
{
    ...
    while (!quit) {
        // Wait until each buffer is full, then write it to the file
        {
            ...
            if (!global_quit) {
                // Test sequence number
                unsigned seq_num = (unsigned char)buffer2[0];
                seq_num |= ((unsigned)(unsigned char)buffer2[1]) << 8;
                seq_num |= ((unsigned)(unsigned char)buffer2[2]) << 16;
                seq_num |= ((unsigned)(unsigned char)buffer2[3]) << 24;
                if (seq_num != expected_seq_num) {
                    std::cout << "Expected sequence " << expected_seq_num;
                    std::cout << " but received " << seq_num << std::endl;
                    expected_seq_num = seq_num + 1;
            ...
            if (!global_quit) {
                // Test sequence number
                unsigned seq_num = (unsigned char)buffer3[0];
                seq_num |= ((unsigned)(unsigned char)buffer3[1]) << 8;
                seq_num |= ((unsigned)(unsigned char)buffer3[2]) << 16;
                seq_num |= ((unsigned)(unsigned char)buffer3[3]) << 24;
                if (seq_num != expected_seq_num) {
                    std::cout << "Expected sequence " << expected_seq_num;
                    std::cout << " but received " << seq_num << std::endl;
                    expected_seq_num = seq_num + 1;
